...
galleryListItem.insertAdjacentHTML("beforeend", createMarkup((0, _galleryItemsJs.galleryItems)));
const galleryLinks = document.querySelectorAll(".gallery__link");
galleryLinks.forEach((galleryLink)=>{
    galleryLink.addEventListener("click", function(clickEvent) {
        clickEvent.preventDefault();
        if (clickEvent.target.tagName !== "IMG") return;
        const bigImage = {
          ...
          <img src="${bigImage.original}" alt="${bigImage.description}" width="800px">
            ...
            onShow: (instance)=>{
                const escapeWindow = (escEvent)=>{
                    if (escEvent.key === "Escape") instance.close();
                };
